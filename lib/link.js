    ...
    beforeRetry: [
      (options, error, retryCount) => {
        ...
        (retryCount > 1 ? logger.retry.warn : logger.retry.info)
          .call(logger.retry, retryCount, url, error.name, error.code, error.event, error.message);
      }
    ]
  },
  agent: {
...
get.defaults.options.dnsCache.fallbackTtl = 10;
  ...
  for (let i = 0; i < 25; i++) {
    ...
    try {
      ...
      if (!res || !res.body || !res.body.length) {
        logger.retry.warn(i, url, 'manually retry on empty response or body', res && res.body);
        continue;
      }
      break;
    } catch (e) {
...
module.exports.dnsCache = get.defaults.options.dnsCache;
